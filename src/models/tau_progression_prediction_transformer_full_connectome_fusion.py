...
class TransformerModelFullConnFusion(pl.LightningModule):
    """
    ...
    def forward(self, src: Tensor, src_mask: Tensor, padding_masks: Tensor, connectome: Tensor) -> Tensor:
        ...
        """
        conn_enc = self.connectome_enc_layer(src) # (seq_len, batch_size, d_in)
        src = torch.cat((src, conn_enc), dim=2) # (seq_len, batch_size, d_in*2)
