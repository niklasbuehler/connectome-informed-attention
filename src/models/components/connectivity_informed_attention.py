...
class ConnectivityInformedAttention(pl.LightningModule):
    ...
    def forward(self, input) -> Tensor:
        ...
        """
        after_connectivity = torch.matmul(input[:,2:202], self.connectivity)
        after_connectivity = nn.functional.normalize(after_connectivity)
        input[:, 2:202] = after_connectivity
    ...
    def step(self, batch: Any):
        x, y = batch
        max_indices = torch.argmax(y[:, 2:202], dim=1)
        identity = torch.eye(200, device=x.device)
        target = identity[max_indices]
        logits = self.forward(x)
        loss = self.criterion(logits, target)
        preds = torch.argmax(logits, dim=1)
        return loss, preds, max_indices
    ...
    def validation_step(self, batch, batch_idx):
        loss, preds, targets = self.step(batch)
        acc = self.train_acc(preds, targets.type(torch.int32))
