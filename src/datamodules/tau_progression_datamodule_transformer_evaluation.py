...
class TauProgressionDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, sequence_dataset: pd.DataFrame) -> None:
        ...
        for id in seq_ids:
            ...
            x = data.loc[:len(data) - 2].iloc[:, 0:-1].to_numpy().astype(np.float32)
            y = data.loc[len(data) - 1].iloc[2:202].to_numpy().astype(np.float32)
            dx_x = data.loc[:len(data) - 2].iloc[:, -1].to_numpy().astype(np.float32)
            dx_y = data.loc[len(data) - 1].iloc[-1].astype(np.float32)
            self.sequences.append((x, y, dx_x, dx_y))

    def __getitem__(self, index):
        sequence = self.sequences[index][0]
        target = self.sequences[index][1]
        input_classes = self.sequences[index][2]
        target_class = np.array(self.sequences[index][3])

        return torch.from_numpy(sequence), torch.from_numpy(target), torch.from_numpy(input_classes), torch.from_numpy(target_class)
...
class TauProgressionDataModuleTransformerEval(LightningDataModule):
    def __init__(
            self,
            data_dir: str = "data/",
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            dataset = df_column_switch(dataset, "DX", "days_to_next")
...
def collate(data, max_len=None):
    ...
    batch_size = len(data)
    if batch_size>1:
        features, labels, input_classes, target_class = zip(*data)
    else:
        features, labels, input_classes, target_class = data[0]
        features, labels, input_classes, target_class = [features], [labels], [input_classes], [target_class]
    ...
    input_classes = torch.stack(input_classes, dim=0)
    target_classes = torch.stack(target_class, dim=0)

    padding_masks = padding_mask(torch.tensor(lengths, dtype=torch.int16),
                                 max_len=max_len)  # (batch_size, padded_length) boolean tensor, "1" means keep

    return X, targets, input_classes, target_classes, padding_masks
...
if __name__ == '__main__':
    import os
    os.chdir("/vol/chameleon/users/derbel/connectome-based-tau-spread-prediction")
    import pandas as pd
    path = "data/tau_progression_sequences_eval.csv"
    df = pd.read_csv(path)
    ...
    ds = TauProgressionDataModuleTransformerEval(batch_size=1, max_len=10, dataset=df)
    ds.setup(stage="test")
