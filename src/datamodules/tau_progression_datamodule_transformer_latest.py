...
class TauProgressionDataModuleTransformerLatest(LightningDataModule):
    def __init__(
            self,
            data_dir: str = "data/",
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            dataset = pd.read_csv(os.path.join(self.hparams.data_dir, self.hparams.dataset_filename)).drop(columns=["ID", "ses"])
            ...
            train_set, val_set, test_set = (dataset[(dataset.SEQ_ID.isin(train_ids))],
                                            dataset[(dataset.SEQ_ID.isin(val_ids))],
                                            dataset[(dataset.SEQ_ID.isin(test_ids))])
