...
class TransformerModelFullConnLateFusion(pl.LightningModule):
    """
    ...
    def __init__(self,
        ...
        self.linear_mapping = nn.Linear(
            in_features=(d_model*max_len + 203*max_len),
            out_features=200
        )
    ...
    def forward(self, src: Tensor, src_mask: Tensor, padding_masks: Tensor, connectome: Tensor) -> Tensor:
        ...
        """
        conn_enc = self.connectome_enc_layer(src) # (seq_len, batch_size, 203)
        conn_enc = self.activation(conn_enc)
        conn_enc = conn_enc.permute(1, 0, 2) # (batch_size, seq_len, 203)
        conn_enc = conn_enc.reshape(conn_enc.shape[0], -1) # (batch_size, seq_len*203)
        ...
        output = torch.cat((output, conn_enc), dim=1) # (batch_size, seq_len*d_model + seq_len*203)
